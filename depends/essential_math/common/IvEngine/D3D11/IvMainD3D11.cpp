...
wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
   ...
   _CrtSetDbgFlag( _CRTDBG_ALLOC_MEM_DF | _CRTDBG_LEAK_CHECK_DF );
    ...
    if (!InitWindow(L"Cali", width, height, fullscreen))
    {
        IvGame::Destroy();
        return 1;
    }
    ...
    // set up renderer
    if (!IvRendererD3D11::Create(gDevice, gContext, gRenderTargetView, gDepthStencilView, gSwapChain)
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	AdditionalInputHandler* handler = GetAdditionalInputHandler();
    ...
    case WM_SIZE:
        ...
        if (IvRenderer::mRenderer)
        {
            ...
            IvRenderer::mRenderer->SetViewPort(width, height);
        }
        break;
