...
#include "D3D11\IvRenderTextureD3D11.h"
...
//-------------------------------------------------------------------------------
IvVertexShader* 
IvResourceManagerD3D11::CreateVertexShaderFromFile( const char* filename, const char* functionname)
{
    IvVertexShaderD3D11* vertexShader = new IvVertexShaderD3D11();
    if ( !vertexShader->CreateFromFile( filename, functionname, mDevice) )
    {
        delete vertexShader;
        vertexShader = 0;
    }
...
//-------------------------------------------------------------------------------
IvFragmentShader* 
IvResourceManagerD3D11::CreateFragmentShaderFromFile( const char* filename, const char* functionname )
{
    IvFragmentShaderD3D11* fragmentShader = new IvFragmentShaderD3D11();
    if ( !fragmentShader->CreateFromFile( filename, functionname, mDevice ) )
    {
...
{
    IvTextureD3D11* texD3D11 = reinterpret_cast<IvTextureD3D11*>(tex);
    texD3D11->Destroy();
    delete texD3D11;
}

IvRenderTexture * IvResourceManagerD3D11::CreateRenderTexture(unsigned int width, unsigned int height, IvTextureFormat format)
{
	IvRenderTextureD3D11* new_render_texture = new IvRenderTextureD3D11;
	if (new_render_texture)
	{
		new_render_texture->Resize(width, height, format, *this);
	}

	return new_render_texture;
}

IvRenderTexture * IvResourceManagerD3D11::CreateRenderTexture(unsigned int width, unsigned int height, unsigned int depth, IvTextureFormat format)
{
	IvRenderTextureD3D11* new_render_texture = new IvRenderTextureD3D11;
	if (new_render_texture)
	{
		new_render_texture->Resize(width, height, depth, format, *this);
	}

	return new_render_texture;
}

void IvResourceManagerD3D11::Destroy(IvRenderTexture * renderTex)
{
	IvRenderTextureD3D11* renderTexD3D11 = static_cast<IvRenderTextureD3D11*>(renderTex);
	renderTexD3D11->Destroy();
	delete renderTex;
}
...
IvConstantBuffer * IvResourceManagerD3D11::CreateConstantBuffer(size_t size)
{
	auto* cbuffer = new IvConstantBufferD3D11;
	if (!cbuffer->Create(mDevice, size))
	{
		delete cbuffer;
		return nullptr;
	}
