...
IvUniformD3D11::IvUniformD3D11(int textureUnit, int samplerUnit,
                             IvShaderProgramD3D11* shader, IvPipelineStage stage)
...
void IvUniformD3D11::SetValue( IvTexture* value )
{
    ...
    mValue.mTexture = dynamic_cast<IvTextureD3D11*>(value);

    if (mShader == IvRenderer::mRenderer->GetShaderProgram())
    {
        mValue.mTexture->MakeActive(
			(unsigned int) mTextureUnit, 
			(unsigned int) mSamplerUnit, 
			static_cast<IvRendererD3D11*>(IvRenderer::mRenderer)->GetDevice(),
			mStage);
        mNeedsUpdate = false;
    }
    else
...
void IvUniformD3D11::Unbind()
{
	if (mType != kTextureUniform) return;
	auto* texture = dynamic_cast<IvTextureD3D11*>(mValue.mTexture);
	texture->Unbind((unsigned int)mTextureUnit);
}
