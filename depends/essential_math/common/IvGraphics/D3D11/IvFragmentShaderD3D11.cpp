...
//-------------------------------------------------------------------------------
bool
IvFragmentShaderD3D11::CreateFromFile(const char* filename, const char* functionname, ID3D11Device* device)
{
    ...
    if (FAILED(D3DCompileFromFile(wstrTo.c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, functionname, "ps_4_0",
    ...
    if (errorMessages)
    {
		...
		DEBUG_OUT("Pixel shader warnings: ");
		DEBUG_OUT(errors << std::endl);
    ...
    if (SUCCEEDED(result))
    {
        mConstantTable = IvConstantTableD3D11::Create(device, code, IvPipelineStage::kPixelStage);
    }
...
IvFragmentShaderD3D11::CreateFromString(const char* string, ID3D11Device* device)
{
    ...
    if (SUCCEEDED(result))
    {
        mConstantTable = IvConstantTableD3D11::Create(device, code, kPixelStage);
    }
    if (FAILED(result) || nullptr == mConstantTable)
    {
        code->Release();
