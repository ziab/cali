...
IvConstantTableD3D11::Create(ID3D11Device* device, ID3DBlob* code, IvPipelineStage stage)
{
...
get_textures:
    ...
    for (int i = 0; i < resourceCount; ++i) 
    {
        ...
        if (bindingDesc.Type == D3D_SIT_TEXTURE && bindingDesc.ReturnType == D3D_RETURN_TYPE_FLOAT
			&& (bindingDesc.Dimension == D3D_SRV_DIMENSION_TEXTURE2D || bindingDesc.Dimension == D3D_SRV_DIMENSION_TEXTURE3D) 
			&& bindingDesc.NumSamples == -1 && bindingDesc.BindCount == 1)
        {
		...
		else if (bindingDesc.Type == D3D_SIT_CBUFFER)
		{
			// just ignore, ok?
			continue;
		}
