                           ...
                           ID3D11Device* device)
{
    ...
    vertexBufferDesc.ByteWidth = (UINT)(numVertices*kIvVFSize[format]);
    switch (usage)
    {
    default:
    case kDefaultUsage:
...
IvVertexBufferD3D11::MakeActive(ID3D11DeviceContext* context)
{
    ...
    UINT stride = (UINT) kIvVFSize[mVertexFormat];
    UINT offset = 0;
...
IvVertexBufferD3D11::EndLoadData()
{
    ...
    if (kDefaultUsage == mUsage)
    {
        ...
        d3dContext->UpdateSubresource(mBufferPtr, 0, nullptr, mDataPtr, 
			(UINT)(mNumVertices*kIvVFSize[mVertexFormat]), (UINT)(mNumVertices*kIvVFSize[mVertexFormat]));
    }
