                      ...
                      void* data, IvDataUsage usage, ID3D11Device* device)
{
    ...
    if (FAILED(device->CreateTexture2D(&desc, subresourcePtr, (ID3D11Texture2D**)&mTexturePtr)))
    {
        return false;
    }
                               ...
                               void** data, unsigned int levels, IvDataUsage usage, ID3D11Device* device)
{
    ...
    if (FAILED(device->CreateTexture2D(&desc, subresourceData, (ID3D11Texture2D**)&mTexturePtr)))
    {
        return false;
    }
...
void IvTextureD3D11::MakeActive(unsigned int textureUnit, unsigned int samplerUnit, 
                               ID3D11Device* device, IvPipelineStage stage)
{
	...
	switch (stage)
	{
	case IvPipelineStage::kPixelStage:
		d3dContext->PSSetSamplers(samplerUnit, 1, &samplerState);
		d3dContext->PSSetShaderResources(textureUnit, 1, &mShaderResourceView);
		break;
	case IvPipelineStage::kVertexStage:
		d3dContext->VSSetSamplers(samplerUnit, 1, &samplerState);
		d3dContext->VSSetShaderResources(textureUnit, 1, &mShaderResourceView);
		break;
	default:
		break;
	}
...
void IvTextureD3D11::Unbind(unsigned int textureUnit)
{
	ID3D11DeviceContext* d3dContext = ((IvRendererD3D11*)IvRenderer::mRenderer)->GetContext();
	ID3D11ShaderResourceView* null = { nullptr };
	d3dContext->PSSetShaderResources(textureUnit, 1, &null);
}
