						...
						IDXGISwapChain* swapChain)
{
    ...
    if (!mRenderer)
    {
        mRenderer = new IvRendererD3D11(device, context, renderTarget, depthStencilTarget, swapChain);
    }
    return ( mRenderer != 0 );
...
//-------------------------------------------------------------------------------
void 
IvRendererD3D11::SetViewPort(unsigned int width, unsigned int height )
{
	...
	if (mOnResizeCbk) mOnResizeCbk(width, height);
}   // End of IvRendererD3D11::Resize()

void IvRendererD3D11::RegisterOnResizeCbk(tOnResizeCbk callback)
{
	mOnResizeCbk = callback;
}
    ...
    ID3D11DepthStencilView* depthStencilTarget)
{
	...
	mContext->OMSetRenderTargets(1, &mRenderTarget, mDepthStencilTarget);
}
...
//-------------------------------------------------------------------------------
// @ IvRendererD3D11::ClearBuffersHelper()
//-------------------------------------------------------------------------------
// Clear necessary buffers
//-------------------------------------------------------------------------------
void IvRendererD3D11::ClearBuffersHelper(
	ID3D11RenderTargetView* renderTarget, 
	ID3D11DepthStencilView* depthStencilTarget, 
	...
	const IvVector4& color)
{
	...
	float color_float_array[4] = { color.x, color.y, color.z, color.w };

	switch (buffer)
	{
	case kColorClear:
		mContext->ClearRenderTargetView(renderTarget, color_float_array);
		break;
	case kDepthClear:
		mContext->ClearDepthStencilView(depthStencilTarget, D3D11_CLEAR_DEPTH, mClearDepth, 0);
		break;
...
{
	ClearBuffersHelper(mRenderTarget, mDepthStencilTarget, buffer, mClearColor);
}
...
void IvRendererD3D11::SetConstantBuffer(IvConstantBuffer * constantBuffer, unsigned int index)
{
	auto* constantBufferD3D11 = static_cast<IvConstantBufferD3D11*>(constantBuffer);
	if (constantBufferD3D11)
	{
		constantBufferD3D11->Set(mContext, index);
	}
}

void IvRendererD3D11::UpdateConstantBuffer(IvConstantBuffer * constantBuffer)
{
	auto* constantBufferD3D11 = static_cast<IvConstantBufferD3D11*>(constantBuffer);
	if (constantBufferD3D11)
	{
		constantBufferD3D11->Update(mContext);
	}
}

void IvRendererD3D11::SetRenderTarget(IvRenderTexture* targetTexture, bool noDepthStencil)
{
	auto* targetTextureD3D11 = static_cast<IvRenderTextureD3D11*>(targetTexture);

	ID3D11DepthStencilView* depthStencil = noDepthStencil ? nullptr : mDepthStencilTarget;

	mContext->OMSetRenderTargets(1, targetTextureD3D11->GetPtrToTargetView(), depthStencil);
}

void IvRendererD3D11::SetRenderTargets(const std::vector<IvRenderTexture*>& targetTextures, bool noDepthStencil)
{
	ID3D11DepthStencilView* depthStencil = noDepthStencil ? nullptr : mDepthStencilTarget;

	static const size_t maxViews = 8;
	ID3D11RenderTargetView* targetViewPtrs[maxViews] = {nullptr};

	if (targetTextures.size() > maxViews) return;

	size_t i = 0;
	for (auto* targetTexture : targetTextures)
	{
		auto* targetTextureD3D11 = static_cast<IvRenderTextureD3D11*>(targetTexture);
		targetViewPtrs[i] = targetTextureD3D11->GetTargetView();

		++i;
	}

	mContext->OMSetRenderTargets((UINT)targetTextures.size(), targetViewPtrs, depthStencil);
}

void IvRendererD3D11::ReleaseRenderTarget()
{
	mContext->OMSetRenderTargets(1, &mRenderTarget, mDepthStencilTarget);
}

void IvRendererD3D11::ClearRenderTarget(IvRenderTexture * targetTexture, IvClearBuffer buffer, const IvVector4& color)
{
	if (!targetTexture) return;

	auto* targetTextureD3D11 = static_cast<IvRenderTextureD3D11*>(targetTexture);
	ClearBuffersHelper(targetTextureD3D11->GetTargetView(), mDepthStencilTarget, buffer, color);
}

void IvRendererD3D11::CopyToBackBuffer(IvRenderTexture * targetTexture)
{
	auto* targetTextureD3D11 = static_cast<IvRenderTextureD3D11*>(targetTexture);
	ID3D11Texture2D* backBuffer = nullptr;
	auto hr = mSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(&backBuffer));
	mContext->CopyResource(backBuffer, targetTextureD3D11->GetTexture());
	backBuffer->Release();
	backBuffer = nullptr;
}
...
void IvRendererD3D11::UpdateUniforms()
{
	...
	IvUniform* model = mShader->GetUniform("IvModelMatrix");
	if (model)
	{
		model->SetValue(mWorldMat, 0);
	}
