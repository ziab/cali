...
//-------------------------------------------------------------------------------
bool
IvVertexShaderD3D11::CreateFromFile(const char* filename, const char* functionname, ID3D11Device* device)
{
    ...
    if (FAILED(D3DCompileFromFile(wstrTo.c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, functionname, "vs_4_0",
    ...
    if (SUCCEEDED(result))
    {
        mConstantTable = IvConstantTableD3D11::Create(device, code, IvPipelineStage::kVertexStage);
    }
...
IvVertexShaderD3D11::CreateFromString(const char* string, ID3D11Device* device)
{
    ...
    if ( FAILED(D3DCompile( string, strlen(string)+1, nullptr, nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "vs_main", "vs_4_0",
                               flags, 0, &code, &errorMessages )) )
    {
        const char* errors = reinterpret_cast<const char*>(errorMessages->GetBufferPointer());
        DEBUG_OUT("Vertex shader error: ");
